...
parser.add_argument('--pbound', type=str, default='0.7')
...
unlabeled_traj = expert_traj[num_label:, :]
...
for i_episode in range(args.num_epochs):
    ...
    unlabeled_num = min(unlabeled_traj.shape[0], num_steps)
    idx = np.random.randint(0, unlabeled_traj.shape[0], unlabeled_num)
    unlabeled_state_action = torch.Tensor(unlabeled_traj[idx, :]).to(device)
    real = discriminator(unlabeled_state_action)

    labeled_num = min(expert_traj.shape[0], num_steps)
    idx = np.random.randint(0, expert_traj.shape[0], labeled_num)    
    ...
    expert_pvalue = torch.Tensor(expert_pvalue).to(device)

    state_action = torch.cat((states, actions), 1).to(device)
    fake = discriminator(state_action)
    real_neg = discriminator(expert_state_action)

    disc_optimizer.zero_grad()
    weighted_loss = nn.BCEWithLogitsLoss(weight=(1-expert_pvalue))
    
    disc_loss = disc_criterion(fake, torch.ones(states.shape[0], 1).to(device)) * Z + \
                weighted_loss(real_neg, torch.ones(expert_state_action.size(0), 1).to(device)) * (1 - Z) / (1 - expert_conf.mean()) + \
